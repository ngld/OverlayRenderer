...
def find_basedir():
    ...
    while not os.path.isfile(os.path.join(base, '.gn')):
...
def main(update_hashes=False):
    ...
    for key, meta in deps.items():
        if 'hash' in meta and key in old and cache[key].get('hash', (None, None))[1] != meta['hash'][1]:
            outdated.add(key)
    ...
    try:
        if missing | outdated:
            ...
            for i, key in enumerate(missing | outdated):
                ...
                if update_hashes and meta['hash'][1] == cache.get(key).get('hash', (None, None))[1]:
                    continue
